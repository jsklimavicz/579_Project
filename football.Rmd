...
There appears to be a statistically significant effect from the number of penalty kicks awarded to the away team, but the effect is quite small, with an average reduction of 0.052 goals per game for each penalty kick awarded to the away team. 
...
Here, we expected to see a positive correlation between $HF$ and $AC$ with the number of away team goals scored. However, these effects were not statistically significant, and instead, we find that $HC$ and $AF$ are statistically significant linear predictors, with decreases of 0.088 and 0.052 goals per game per event, respectively. There is also a statistically significant interaction between $HC$ and $AF$. 
...
Most of the parameters are significant at the $\alpha = 0.05$ level in this model. $HC$ and $AF$ were associated with higher numbers of home team goals, while $HF$ and $AC$ were associated with lower numbers of home team goals. 
...
The residuals are also quite substantial. Indeed, the mean absolute residual was `r round(abs_res_mean,2)` goals, which is nearly as large as the mean number of goals scored by the home team, which was `r round(FTHG_mean,2)` goals. As a result, we conclude that while more penalty kicks and corner kicks are associated with lower home team, away team, and total goals scored, likely due to less playing time from game stoppage, the number of these kicks has little predictive power in determining the number of these goals. 
